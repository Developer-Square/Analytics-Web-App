  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<IEventPaginationResult>) => {
...
export const selectCurrentEvent = (_id: string) =>
  createSelector(selectEventEntities, (event) => event[_id]);

export const selectEventsByDateRange = createSelector(
  selectEvents,
  (state: AppState) => state.dateFilters,
  (events, dateFilters) =>
    filterByDateRangeUsingEmbeddedField(
      events,
      dateFilters.finalDate,
      dateFilters.initialDate,
      dateFilters.outerField,
      dateFilters.filterKey
    )
);
...
export const selectMultipleFilteredSortedEvts = createSelector(
  ...
  (events, eventMultipleFilters, sortingFilters) =>
    filterByEnumAndSort(
      events,
      eventMultipleFilters,
      'event',
      sortingFilters.ascending,
      'meta',
      sortingFilters.sortKey
    )
);
...
export const selectEventsPerPage = (state: AppState) =>
